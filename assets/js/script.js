const plus = document.getElementById('plus');
        ...
        function createNote(color) {
            ...
            noteContainer.innerHTML = `
                <textarea class="note ${color}" id="note" cols="30" rows="10" onchange="handleChange(this.value, ${index + 1})"></textarea>
                <div class="date-time">${new Date().toDateString()} ${new Date().toTimeString().split(" ")[0]}</div>
                <button class="button ${color}" id="note-${index + 1}" onclick="deleteNote(this.id)"><i class="fas fa-trash"></i></button>
            `;
            ...
            document.getElementsByClassName('notes-container')[0].appendChild(noteContainer);
            index++;
        }
            ...
            if (colors) {
                ...
                for (let key in colorsObj) {
                    if (notesObj === null) {
                        ...
                        noteContainer.innerHTML = `
                        <textarea class="note ${colorsObj[key]}" id="note" cols="30" rows="10" onchange="handleChange(this.value, ${key.split('-')[1]})"></textarea>
                        <div class="date-time">${new Date().toDateString()} ${new Date().toTimeString().split(" ")[0]}</div>
                        <button class="button ${colorsObj[key]}" id="${key}" onclick="deleteNote(this.id)"><i class="fas fa-trash"></i></button>
                    `;
                        ...
                        noteContainer.id = key;
                        noteContainer.innerHTML = `
                        <textarea class="note ${colorsObj[key]}" id="note" cols="30" rows="10" onchange="handleChange(this.value, ${key.split('-')[1]})">${notesObj[key]===undefined ? "" : notesObj[key]}</textarea>
                        <div class="date-time">${new Date().toDateString()} ${new Date().toTimeString().split(" ")[0]}</div>
                        <button class="button ${colorsObj[key]}" id="${key}" onclick="deleteNote(this.id)"><i class="fas fa-trash"></i></button>
                    `;
                        noteContainer.style.backgroundColor = colorsObj[key];
